...
public class EmployeeSearch {
  ...
  private static final String PREFIX_IN_JAR = "WithPostProcessing/";
  ...
  public void withPostprocessing() throws URISyntaxException, IOException {
    ...
    createDirectory(scheduleTestDir + "/" + PREFIX_IN_JAR + "/");
    createDirectory(scheduleTestDir + "/" + PREFIX_IN_JAR + "/bin");
    createDirectory(scheduleTestDir + "/" + PREFIX_IN_JAR + "/js");
    createDirectory(scheduleTestDir + "/" + PREFIX_IN_JAR + "/lib");
    createDirectory(scheduleTestDir + "/" + PREFIX_IN_JAR + "/css");
    ...
    final String ics2html = FileExtractorFromJar.with(PREFIX_IN_JAR + "bin/ics2html", scheduleTestDir).getPath();
    logger.info("ics2html {}", ics2html);
    executableFlagTo(ics2html);

    final String ical2html = FileExtractorFromJar.with(PREFIX_IN_JAR + "bin/ical2html", scheduleTestDir).getPath();
    logger.info("ical2html {}", ical2html);
    executableFlagTo(ical2html);

    FileExtractorFromJar.with(PREFIX_IN_JAR + "css/calendar.css", scheduleTestDir).getPath();

    FileExtractorFromJar.with(PREFIX_IN_JAR + "js/jquery.min.js", scheduleTestDir).getPath();
    FileExtractorFromJar.with(PREFIX_IN_JAR + "js/style_cols.js", scheduleTestDir).getPath();

    final String libical = FileExtractorFromJar.with(PREFIX_IN_JAR + "lib/libical.so.1.0.1", scheduleTestDir).getPath();

    String preload = "LD_PRELOAD=" + libical + " ";
    final String[] command = {
        "/bin/bash", "-c",
        preload + " " + ics2html + " " + scheduleTestDir + " " + scheduleTestDir + "/" +
            PREFIX_IN_JAR + "/bin" +
            " " + PREFIX_IN_JAR
    };
  ...
  private void executableFlagTo(String ics2html) throws IOException {
    Set<PosixFilePermission> permissions = new HashSet<>();
    permissions.add(PosixFilePermission.OWNER_EXECUTE);
    permissions.add(PosixFilePermission.OWNER_READ);
    permissions.add(PosixFilePermission.OWNER_WRITE);
    Files.setPosixFilePermissions(new File(ics2html).toPath(), permissions);
  }
