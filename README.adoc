...
image::https://travis-ci.org/rogerGunis/scheduler2ics.svg?branch=master[asdf]
...
### holidays
see link:src/test/resources/WithPostProcessing/inputData/Holidays.csv[Holidays.csv]
```
...
### jobDescriptions
see link:src/test/resources/WithPostProcessing/inputData/JobDescription.csv[JobDescription.csv]
```
...
### workers
link:src/test/resources/WithPostProcessing/inputData/Workers.csv[Workers.csv]
```
...
The link:src/test/resources/WithPostProcessing/bin/IcsFileSync.sh[IcsFileSync.sh] will check previously deployed entries
